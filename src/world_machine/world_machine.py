...
class WorldMachine(torch.nn.Module):
    def __init__(self, state_size:int, max_context_size:int,
        ...
        self._positional_encoder = SinePositionalEncoding(state_size, max_context_size)
    ...
    def forward(self, state:torch.Tensor, 
        ...
        if sensorial_masks is None:
            ...
            for name in sensorial_data.keys():
                sensorial_masks[name] = torch.ones((batch_size, seq_len), dtype=bool, device=device)
